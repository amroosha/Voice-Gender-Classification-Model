...
│   └── ood_test/                 # separate OOD dataset, never used in training
├── models/                       # bundled preprocessor+classifier artifact, wav2vec2 config
├── src/
│   ├── __init__.py
│   ├── config.py                  # paths, SAMPLE_RATE=16000, WAV2VEC2_MODEL_NAME,
│   │                                WAV2VEC2_REVISION, WAV2VEC2_LAYER, MIN_AUDIO_DURATION_S, DEVICE
│   ├── audio_utils.py             # in-memory loading (torchaudio), resampling, mono, validation
│   ├── feature_extractor.py       # frozen wav2vec2 embedding extraction
│   └── train.py                   # loads data, extracts/caches embeddings, trains + evaluates,
│                                     logs to MLflow, runs OOD eval, exports final artifact
├── app/
│   ├── __init__.py
│   ├── main.py                    # FastAPI app, lifespan model loading, /health, /v1/predict-gender
│   └── schemas.py                  # Pydantic response models
├── tests/
│   ├── test_api.py
│   ├── test_audio_utils.py
│   └── test_feature_extractor.py
├── Dockerfile                      # multi-stage, uv-based, python:3.11-slim + libsndfile1 + ffmpeg
├── pyproject.toml
├── uv.lock
└── README.md                       # uv sync, train + mlflow ui, docker build, curl examples
```

## 6. Decisions already settled — do not relitigate without a real reason
